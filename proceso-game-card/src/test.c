...
void crear_bloques_con_filesystem_vacio(){
	...
	int bloque_nuevo = agregar_registro_a_bloque(posicion,cantidad,bloque_inicial);
	if(bloque_nuevo != -1){
	...
	bloque_nuevo = agregar_registro_a_bloque(posicion,cantidad,bloque_inicial);
	if(bloque_nuevo != -1){
		printf("Tengo asignado el bloque 5 y el bloque %d\n",bloque_nuevo);
		bloque_inicial = bloque_nuevo;
	}else{
...
void leer_posiciones_de_bloques_con_filesystem_inicializado(){
	...
	t_list* posiciones_en_bloque = obtener_posiciones_de_bloques(bloques_asignados);
