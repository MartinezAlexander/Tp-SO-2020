...
t_catch_pokemon* catch_pokemon_create(char* nombre, uint32_t posX, uint32_t posY){
	...
	catch_pokemon->tamanio_nombre = strlen(nombre);
...
t_buffer* catch_pokemon_to_buffer(t_catch_pokemon* catch_pokemon){
	...
	buffer->stream =  catch_pokemon_to_stream(catch_pokemon);
	return buffer;
}

t_catch_pokemon* catch_pokemon_from_buffer(t_buffer* buffer){
	return catch_pokemon_from_stream(buffer->stream);
}
...
void* catch_pokemon_to_stream(t_catch_pokemon* catch_pokemon){
	int size = sizeof(uint32_t) * 3 + catch_pokemon->tamanio_nombre;
...
t_catch_pokemon* catch_pokemon_from_stream(void* stream){
	...
	catch_pokemon->pokemon->especie = malloc(catch_pokemon->tamanio_nombre + 1);
	memcpy(catch_pokemon->pokemon->especie, stream,catch_pokemon->tamanio_nombre);
	catch_pokemon->pokemon->especie[catch_pokemon->tamanio_nombre + 1] = '\0';
